...
.. tip::
   Python operator 模块提供了全部比较操作以及数值操作。该模块主要用于：函数化操作符，将一些常见的操作符封装成函数，以便在函数式编程中使用。这样可以避免使用 lambda 函数或编写额外的函数来执行这些操作；用于高阶函数，在某些高阶函数（例如 map、filter、functools.reduce 等）中，通过使用 operator 模块的函数，可以提高代码的简洁性和可读性。
